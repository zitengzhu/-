...
# res_6 = obj.findall(s)
# print(res_6)   ["<span id='10010'>中国联通</span>", "<span id='10086'>中国移动</span>"]

# 加入小括号的元字符 可以筛选信息单独提取内容 使目标数据更简洁
# obj = re.compile("r<span id='(\d+)'>(.*?)</span>")
# res_6 = obj.findall(s)
# print(res_6)     [('10010', '中国联通'), ('10086', '中国移动')]
...
'--------------替换字符串-----------------------'
obj = re.compile(r'1[34578]\d{9}')
data = re.sub(obj,'1xxxxxxxx','中奖号码为:84978981 联系电话为13611111111')
# print(data)   中奖号码为:84978981 联系电话为1xxxxxxxx
# 新写法效果同上
res = re.sub(r'1[34578]\d{9}','1xxxxxxxx','中奖号码为:84978981 联系电话为13611111111')

'---------------使用正则表达式切割字符串------------------------------'
# split()方法用于实现正则表达式分割字符串，并以列表的形式返回。

obj = re.compile(r'\s*@')
list = re.split(obj,'@明日科技 @别人噶此 @小米')
print(list)   # ['', '明日科技', '别人噶此', '小米']
'''
@ 符号作为分隔符：当正则表达式以 @ 作为分隔符进行切割时，split() 方法会在每个 @ 的位置进行分割
空字符串的产生：如果文本中有连续的 @ 符号，或者 @ 出现在开头/结尾，切割后就会产生空字符串或只包含空格的字符串
'''
# 想要删除空元素可以使用循环和判断
for i in list:
    if i != '':
        print(i,end=' ')  # 明日科技 别人噶此 小米
